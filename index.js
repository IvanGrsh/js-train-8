...
function getUsersFromDatabase(array) {
  const arr = Array.from(array, (record) => {
    return {
      id: record.id,
...
function findElementIndexes(arr, element) {
  if (Array.isArray(arr)) {
    const find1 = arr.indexOf(element);
    const find2 = arr.lastIndexOf(element);
    const newArr = [];
    if (find1 !== -1 && find2 !== -1) {
      newArr.push(find1, find2);
    }
    return newArr;
  }
  return;
...
function copyAndSwapElements(arr, target, start, end) {
  if (Array.isArray(arr, target, start, end) > 0) {
    const cop = arr.copyWithin(target, start, end);
    return cop;
  } else {
...
function customEvery(arr, condition) {
  if (
    Array.isArray(arr).length === true &&
    Array.isArray(condition) !== "function"
  ) {
    return "false";
  } else {
...
function customShift(arr) {
  if (Array.isArray(arr) === true && Array.isArray(arr) > 0) {
...
function customUnshift(arr, ...elements) {
  ...
  //   if (Array.isArray(arr) === true) {
  //     const newLength = elements.unshift(arr);
  //     return elements;
  //   }
...
function customAt(arr, index) {
  if (Array.isArray(arr) === true && index <= arr.length) {
    let testElem = typeof arr[index];
    if (testElem === "number") {
      console.log("Елемент є числом");
      return arr[index];
    } else if (testElem === "string") {
      console.log("Елемент є рядком");
      return arr[index];
    } else if (testElem === "object") {
...
function customIncludes(arr, element) {
  if (Array.isArray(arr) === true && arr.includes(element) === true) {
    const arr1 = (value) => value === element;
    const arr2 = arr.filter(arr1);
